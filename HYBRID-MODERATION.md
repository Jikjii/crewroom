...
**Live September 22, 2026 EDT (September 23 UTC).** Hybrid photo/text screening is active on Render at backend revision `1f3d931`. Sightengine Starter ($29/month) is confirmed active and its existing credentials are stored only in the private server environment. The owner's personal Jikjii account has verified browser moderation access. New public submissions publish after successful automatic checks; videos and flagged/incomplete submissions stay under human review. Existing pending submissions remain pending. Signed iPhone preview 0.4.0 (4) is ready for physical testing and has not replaced the private TestFlight release.
...
Create an image workflow and, if enabling video automation, a separate video workflow in Sightengine. Cover explicit sexual content, graphic injury, hate symbols, violence, self-harm and harmful text embedded in media. The prepared adapter lists its expected models in `server/sightengine.mjs`. The image workflow uses five visual models (`nudity-2.1`, `gore-2.0`, `offensive-2.0`, `violence`, `self-harm`); disable legacy workflow Text Analysis. Crewroom separately calls `ocr,text-content-2.0` with explicit harmful-text categories and Latin-script OCR (`opt_lang=en`) for every accepted image and poster. Nonempty extracted text also passes through the contextual text checks below; it is not logged or retained. Missing visual model scores, missing raw OCR content, malformed results or oversized extracted text stay held. An explicit empty OCR content string plus complete empty detections permits the provider's observed `language:null` no-text result; omitted fields do not establish a clean scan. Every required model must run before an ACCEPT decision; do not add an early ACCEPT branch that skips later checks. Crewroom treats a workflow rejection as a human-review hold, not an automatic permanent deletion. [Image workflow instructions](https://sightengine.com/docs/image-moderation-workflows), [video workflow instructions](https://sightengine.com/docs/video-moderation-workflows)

Text checks combine general ML classification and self-harm rules in English/Spanish, then a separate English-only self-harm ML check. Live requests established that the self-harm ML model rejects Spanish (including `en,es`), despite the broader text API accepting multiple language codes. Spanish rules supplement general classification; they are not equivalent to contextual Spanish self-harm ML. OCR uses Latin-script recognition and does not establish comprehensive multilingual image-text coverage. Language outputs indicate applied rules, not reliable identification of the actual language. Reports and human review remain necessary. [Text models](https://sightengine.com/docs/text-moderation-ml-models), [self-harm rules](https://sightengine.com/docs/text-moderation-rule-based), [OCR scope](https://sightengine.com/docs/ocr-api-text-in-images-and-videos)

A text-free photo normally consumes four visual/OCR operations. A short caption adds approximately two text operations; nonempty recognized image text adds approximately two more per 1,000 characters. Multiple images, long text and retries increase usage. These are operation estimates, not a per-post price guarantee.
...
**Owner decisions recorded:** Starter photo/text screening at $29/month, videos and audio held for human review, and the personal Jikjii account selected for operator access (resolved from the owner-provided email to an immutable production account ID). The owner subsequently confirmed payment, moderator access and transfer of the existing Sightengine API credentials to Crewroom on Render. Subscription activation, operator access and the final live checks are verified. Hybrid mode was then activated with video automation disabled.
...
- Created and subsequently activated the Sightengine image workflow `wfl_lsEyOtDqOdR3u4rZB7xCv` (Crewroom public images v1). Its visual rules cover nudity, graphic injury, violence, self-harm and hate imagery. Flags mean Crewroom human review, never automatic permanent deletion. Costume skin exposure, swimwear, minor cuts/stage blood and prop objects are not blanket bans. Contextually serious matches remain held.
...
- Initial live checks caught two provider differences: Spanish was rejected by the self-harm ML classifier, and clean OCR results contained empty arrays and `language:null`. The image rule filter also missed the synthetic threat. Follow-up diagnostics validated separate multilingual general/rule checks and English self-harm ML, and established that contextual classification of raw OCR detects the image threat. Those corrections require a fresh successful full fixture run before activation.
...
This opt-in check sends only the three hash-pinned fictional demo assets already shipped with Crewroom and synthetic text/OCR fixtures. It never reads user uploads or the production database and changes no production flags. It caps itself at 22 HTTP requests, requires exact pass/hold outcomes, checks that Starter videos make no provider calls, and prints only fixed sanitized result codes. A provider error cannot masquerade as successful harmful-content detection. Model operations can exceed HTTP request count. All eight cases must pass before considering activation, followed by the broader acceptance checks above. This small fixture set does not establish classifier accuracy across cosplay, languages or all safety categories.

- Follow-up Render deployment `dep-dapjue7lk1mc73bsgcag` succeeded at `e813943`, adding explicit OCR 2.0 checks, five-model output validation and the opt-in live test command. That intermediate deployment remained manual until the later activation below.
...
- Configuration deployment `dep-dapk39m7bikc73fpe080` saved the approved personal operator ID and private Sightengine credentials in manual mode. Starter showed ACTIVE. The signed-in owner opened the live Review queue (two pending submissions, zero reports). No existing pending submission was approved or transmitted by the fixture checks.

- The integration corrections passed 60/60 combined adapter, fixture-harness, hybrid-worker and moderation-desk regression tests. The corrected live acceptance run subsequently passed as recorded below.

### Completed activation

- Corrected code deployment `dep-dapkb9rtqb8s73dejta0` at `1f3d931` succeeded in 1m07s.
- The corrected live command passed **8/8 cases in 13 HTTP requests**: harmless caption; three public fictional demo photos; threatening caption; Spanish self-harm caption; threat embedded in an image; and video held without provider requests. The embedded threat was caught by contextual classification after the image rule filter alone missed it. Only fixed public demo assets and synthetic text were used; the production database was not touched by these checks.
- Activation deployment `dep-dapkcqbtqb8s73dep3c0` completed in 24.8s. `MODERATION_MODE=hybrid`, provider approval and workflow verification are true; audio automation remains false and no video workflow is configured.
- Live health returned HTTP 200. The capability endpoint returned `{"mode":"hybrid","automaticScreening":true,"provider":"sightengine","automaticVideoScreening":false}`. Signed-out operator identity returned `allowed:false`; queue requests returned HTTP 401.
- The owner's signed-in moderation workspace reopened successfully with two older pending submissions and zero reports. No pending content was bulk-approved or retried. The live privacy page displayed Sightengine public text/photo processing and the human video-review boundary.
- Existing private TestFlight stays 0.3.0 (2). Candidate 0.4.0 (4) still needs the physical-device checklist before a new TestFlight release. The live fixture set does not establish universal classifier accuracy; daily report review and further cosplay-specific calibration remain operational responsibilities.
