...
**Activation checks in progress September 23, 2026 (UTC).** The moderation desk/backend at `e813943` is deployed on Render in manual mode. Automatic screening is not activated. Signed iPhone preview 0.4.0 (4) includes the new moderation UI and is ready for physical testing; it has not replaced the private TestFlight release. Sightengine Starter ($29/month) is confirmed active; credentials are configured privately on Render. The owner explicitly approved operator access for their personal Jikjii account, and its live moderation desk successfully opened the queue. Videos remain under human review. Keep `MODERATION_MODE=manual` until credentials, operator access, current-model coverage and live acceptance checks are complete.
...
**Owner decisions recorded:** Starter photo/text screening at $29/month, videos and audio held for human review, and the personal Jikjii account selected for operator access (resolved from the owner-provided email to an immutable production account ID). The owner subsequently confirmed payment, moderator access and transfer of the existing Sightengine API credentials to Crewroom on Render. Subscription activation and operator access are verified; automatic screening remains gated on the final live checks.
...
- The integration corrections passed 60/60 combined adapter, fixture-harness, hybrid-worker and moderation-desk regression tests. The corrected live acceptance run is still required before activation.
