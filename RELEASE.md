...
**Current release work:** see [TESTFLIGHT-0.4.0.md](TESTFLIGHT-0.4.0.md) for the video/hybrid-moderation iPhone beta. Build 0.4.0 (5) was Approved September 23 at 11:55 AM EDT and observed **Testing** around 1:16 PM EDT. The owner confirmed public-video playback and camera cancellation in the existing 0.4 iPhone app, then reported the feed resetting each video to muted. A source fix carries the sound preference across the open feed; a signed update and swipe verification are next. The 0.3 status below is a **historical snapshot from September 18, 2026**; its Apple review/tester status is not the current release status.
