...
export function Avatar({
  ...
  return (
    ...
    >
      <Text style={{ color: readableOnColor(color ?? C.lavender), fontWeight: "700", fontSize: size * 0.31 }}>
        {name
          .split(" ")
          .map((n) => n[0])
          .slice(0, 2)
...
const createStyles = (C: Palette) => StyleSheet.create({
  ...
  brand: { fontSize: 26, fontWeight: "800", letterSpacing: -1.3, color: C.ink },
  ...
  h2: { fontSize: 20, letterSpacing: -0.6, fontWeight: "700", color: C.ink },
  ...
  buttonText: { color: C.onAccent, fontSize: 14, fontWeight: "700" },
  ...
  taskTitle: { fontSize: 14, fontWeight: "500", color: C.ink, lineHeight: 20 },
...
export function AppearanceControl() {
  ...
  return (
    <View style={{ gap: 12 }}>
      ...
      <View accessibilityRole="radiogroup" accessibilityLabel="Appearance" style={[s.row, { gap: 8 }]}>
        {([
          ...
          return (
            <Pressable
              ...
              style={[s.chip, {
                flex: 1, gap: 8, minHeight: 80, alignItems: "center", justifyContent: "center",
                paddingHorizontal: 5,
              }, selected && s.chipActive]}
            >
              <Icon name={option.icon} color={selected ? C.blue : C.muted} size={22} />
              <Text style={[s.chipText, selected && { color: C.blue }]}>{option.label}</Text>
            </Pressable>
          );
        })}
      </View>
      <Text style={s.small}>
        {preference === "system" ? `Following your device’s appearance. Currently ${resolved}.` : `${preference === "dark" ? "Dark" : "Light"} mode is on.`}{storageError ? "" : " Saved on this device."}
      </Text>
      {!!storageError && <Text accessibilityRole="alert" style={s.errorText}>{storageError}</Text>}
    </View>
  );
}
