...
import { useModerationPolicy } from "../social/moderationPolicy";
...
export function PublicPageContent({ page, config, onOpen, children }: {
  ...
  return <View style={{ gap: 24 }}>
    ...
    {page === "privacy" ? <>
      ...
      {section("What we store and why", "We store your name, email, a salted password hash, sign-in sessions, and the information you choose to add: your creator profile, posts, photos, videos and their recorded sound, comments, follows, saves, collaboration requests, crew memberships, plans, assignments, and invitations. This lets you sign in, find creators, share work, and coordinate private crews. Report and block records help us handle abuse. If signup consent is required, we record the policy version and your age and terms confirmation.")}
      {section("What other people can see", `Profiles start private. ${moderation.sharingSummary} Public edits are checked again. Published content is available to people without an account and may be indexed or copied. Credits on published posts are public. Shared crews, tasks, lineup and day plans are available to crew members. Collaboration requests are visible to the people involved; accepting one creates a new private shared crew. Your account email and password are not published on your profile.`)}
      {section("Photos, videos and device storage", "Crewroom uploads only photos or videos you choose. In the phone app, camera and microphone access lets you record a video with sound when you choose Record video; recording stops when you leave the camera. Uploaded photos are resized; videos are converted for playback and a preview image is created. Public videos and their audio are subject to safety checks and may need operator review. Original files and embedded location metadata are discarded after processing. Failed upload temporary files are removed. Browser sign-in uses an essential HttpOnly cookie; native sign-in uses secure device storage. Your appearance choice is saved on your device. This beta does not include advertising trackers or sell personal information.")}
      {moderation.automatic && moderation.config?.provider === "sightengine" && section("Automated safety checks", `When you choose public sharing, Crewroom sends submitted public text and photos to Sightengine for automated safety screening. ${moderation.automaticVideo ? "Public video content and recorded sound are also sent for configured video checks; automated audio screening currently covers English." : "Videos and their sound currently need operator review."} Private drafts and private crew plans are not sent for this screening. We store screening outcomes and moderation actions to handle reports, review decisions and prevent abuse. Automated checks can make mistakes; flagged or uncertain content may stay hidden for operator review. Contact support to challenge a decision or ask about screening data.`)}
      {section("Hosting and email", "Our hosting provider processes the service’s database, photos, videos, and operational traffic. When enabled, our email provider processes your email address to deliver password-reset links and requested beta confirmation emails. Launch updates require a separate opt-in. We use IP addresses and hashed email addresses for abuse rate limits. Hosting providers may keep their own access and security logs. Access to administrative tools is restricted to the operator.")}
      {section("Beta access requests · beta-waitlist-1", "Requesting beta access does not create an app account. We store your email, device choice, age confirmation, email preferences and confirmation times, optional creator role and next-shoot description, and campaign labels from the signup link. We use this to manage beta invitations and send launch updates only if you opt in. This list is private to the operator. Confirm your email before your request becomes eligible for invitations. Unconfirmed requests expire 7 days after the latest request; confirmed requests expire 180 days after confirmation. A fresh confirmation link can keep the record until that link expires, up to 7 additional days; reconfirming restarts retention. Use the removal link in your confirmation email to delete your live waitlist details and stop updates, or contact support. This does not delete an existing app account. Expired records are removed during hourly or request-time cleanup; restricted backup copies expire under the backup policy below.")}
      ...
      {section("Your work stays yours", "You retain your rights to your photos, videos, costumes, writing, and other work. You give Crewroom permission to store, resize, convert, play, display and share it according to the visibility and collaboration features you choose, for the purpose of operating the service. Upload only material you own or have permission to share, including permission for music, sound, photographers and identifiable people. Credit contributors accurately. Crewroom does not claim ownership of the characters or franchises represented in cosplay.")}
      ...
      {section("Respect creators and consent", "Welcome cosplayers, photographers, makers and fans across backgrounds, identities, body types and skill levels. No harassment, discrimination, hate, threats, stalking, impersonation, doxxing, scams or unwanted sexual contact. Do not share someone’s private location, schedule or contact details without their consent.")}
      ...
      {section("Cosplay in context", "Costumes, fictional characters, fantasy combat and clearly presented prop-making are part of this community. A prop weapon or revealing costume alone is not a violation. Context, consent and these standards matter. Do not threaten real people, promote dangerous weapon use, or use cosplay as a pretext for hate or exploitation.")}
      ...
      {section("Report a concern", "Use Report on a post, comment or creator profile. You can report harassment, sexual content, threats, hate, child-safety concerns, stolen work, spam or another concern. Block a creator to restrict interaction. For concerns you cannot report in-app, use the support email below; include a link and description, and do not email illegal imagery. Reports go to the operator for review; immediate responses are not guaranteed. For urgent danger, contact local emergency services.")}
      {section("Public sharing and safety", `${moderation.sharingSummary} Changes to public content are checked again. You can see the status and any review note on your own submissions. Published content may later be restricted or removed after reports or further review. Contact support if you disagree with a moderation decision.`)}
